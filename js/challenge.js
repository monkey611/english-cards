...
function playText(text, lang) {
  ...
  try {
    ...
    if (!ok) {
      // 运行中 voices 为空：尝试 audio-player 兜底，避免题目无声
      if (typeof playAudio === 'function') { playAudio(text, lang || 'en-US'); return; }
      return;
    }
    safeSpeak(u);
  } catch (e) {
    // 异常时兜底，保证闯关发音不中断
    if (typeof playAudio === 'function') playAudio(text, lang || 'en-US');
  }
}
